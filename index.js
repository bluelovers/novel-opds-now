...
async function startServer(options = {}) {
    ...
    const web = await (0, http_1.createServer)((0, micro_1.serve)(await Promise.resolve().then(() => tslib_1.__importStar(require('./server/index'))).then(m => m.default)));
    if (!options.port) {
        ...
        process.env.PORT = port = await (0, get_port_1.default)({
            port: (0, get_port_1.portNumbers)(port, (port | 0) + 10),
        });
